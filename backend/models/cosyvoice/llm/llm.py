...
class Qwen2Encoder(torch.nn.Module):
    ...
    def _get_cache_seq_len(self, cache):
        if cache is None:
            return 0
        try:
            ...
            if isinstance(cache, DynamicCache):
                if hasattr(cache, 'layers') and cache.layers:
                    ...
                    if hasattr(first_layer, 'keys'):
                        return first_layer.keys.size(2)
                    elif hasattr(first_layer, 'key'):
                        return first_layer.key.size(2)
        except ImportError:
            pass
        return 0
            ...
            if prompt_speech_token_emb.size(1) == 0:
                if (len(out_tokens) != 0 and out_tokens[-1] == self.fill_token) or (len(out_tokens) == 0 and lm_input.size(1) == 1):
                    ...
                    if text_cache.size(1) >= self.mix_ratio[0]:
                        ...
                        continue
                while True:
                    cache_seq_len = self.llm._get_cache_seq_len(cache)
                    seq_len = lm_input.shape[1] + cache_seq_len
                    y_pred, cache = self.llm.forward_one_step(lm_input,
