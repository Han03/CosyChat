...
def save_audio_history(line_id: int, content: str, role: str, tone: str, instruction: str, agent_id: str, seed: int, audio_path: str):
    import time
    conn = _get_conn()
    with _lock:
        conn.execute(
            """INSERT INTO script_line_audio_history 
               (line_id, content, role, tone, instruction, agent_id, seed, audio_path, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (line_id, content, role, tone, instruction, agent_id, seed, audio_path, time.time())
        )
        conn.commit()
...
def get_matching_audio_history(line_id: int, content: str, role: str, tone: str, instruction: str, agent_id: str, seed: int) -> Optional[Dict[str, Any]]:
    conn = _get_conn()
    with _lock:
        row = conn.execute(
            """SELECT * FROM script_line_audio_history 
               WHERE line_id=? AND content=? AND role=? AND tone=? AND instruction=? AND agent_id=? AND seed=?
               ORDER BY created_at DESC LIMIT 1""",
            (line_id, content, role, tone, instruction, agent_id, seed)
        ).fetchone()
